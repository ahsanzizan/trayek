...
export const podUploadInput = z.object({
   ...
   *
   * A retry after a dropped connection re-sends the same key, which is how
   * the second attempt lands on the first attempt's submission instead of
...
export async function authorizePodUpload({
  ...
  // No submission is opened here. It is opened when the first photograph
  // actually arrives, so a failed upload leaves nothing behind for TRK-041 to
...
export const uploadRouter = {
  podUploader: f({
    ...
    .onUploadComplete(async ({ metadata, file }) => {
      const podSubmissionId = await findOrOpenPodSubmission({
        db,
        organizationId: metadata.organizationId,
        orderId: metadata.orderId,
        podUploadLinkId: metadata.podUploadLinkId,
