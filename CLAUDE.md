...
2. **No new dependency without asking.** This includes `clsx`, `cn`, `class-variance-authority`, a form library, a state library, and a test runner. Propose it and say why the existing tools can't do it.
...
These directories **do not exist yet**. Create one only when its rule fires, and put nothing else in it:

| Need | Home | Create it when |
| --- | --- | --- |
| Component used by 2+ routes | `src/components/` | the second route imports it — not before |
| Pure helper, no React/Next imports | `src/lib/` | a second caller appears |
| Type shared by 2+ modules | `src/types/` | a second module imports it |

Do not invent `utils/`, `helpers/`, `common/`, `shared/`, or `services/`. Those names attract unrelated code and are how a tree turns to mush. If something genuinely fits none of the homes above, ask.
...
- **Theme tokens go in the `@theme` block of `src/styles/globals.css`.** Tailwind v4 is CSS-first: there is no `tailwind.config.ts` and creating one will not take effect.
- Prefer a token over an arbitrary value (`w-[123px]`) unless nothing reasonable matches.
- `prettier-plugin-tailwindcss` sorts class names on format — don't hand-order them or fight the output.
- Repeated class strings: extract a component, not a class-name helper. `clsx`/`cva` are not installed (see anti-spaghetti rule 2).
...
- Don't create `tailwind.config.ts`; Tailwind v4 configures through `@theme` in `src/styles/globals.css`.
